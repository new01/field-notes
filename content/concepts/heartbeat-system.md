...
Getting the heartbeat right is one of the highest-leverage things you can do in your OpenClaw setup. Getting it wrong means either a passive agent that never does anything proactively, or an overactive one that burns context and costs money on useless work.
...
The interval is in minutes. 30 minutes is a reasonable default — frequent enough to catch issues promptly, infrequent enough to not burn context budget.

### Multiple Heartbeats

You can configure different heartbeats at different intervals for different purposes. A 30-minute operations check pairs with an hourly Phase 2 dispatch cycle:
...
5. If any alerts: DM Swifty. If all clear: reply HEARTBEAT_OK.
```

### The inline-only rule

Phase 1 must not spawn subagents. Phase 1 is the health check — if the health check itself spawns agents, it creates recursive load and confuses the monitoring picture.

All Phase 1 work must complete in a single turn. If a check requires 10 tool calls, it's too heavy for Phase 1.
...
Phase 2 only fires when conditions are right. Key condition: the human is inactive.
...
3. If last message > 30min ago:
   ...
   d. Spawn Claude Code session with: "Execute spec at builds/<id>/spec.md. Report completion."
   e. Update item status to "in-progress"
   f. Reply with: "Dispatched [item title] — monitoring"
```
...
- DM the human about debug output. If the dispatched task has problems, log them — don't ping unless there's an actionable decision required.
- Run multiple heavy agents simultaneously. One dispatch per heartbeat cycle.
- Do work that takes longer than 5 minutes inline. Everything heavy goes to a spawned session.

---

## Heartbeat vs Cron

People often ask: should I use a heartbeat or a cron for this?
...
- You want a different model or temperature for the task
- The task is self-contained with direct delivery (morning brief → Telegram, no main session needed)
- You want the work to happen whether or not the main session is open

**The practical heuristic:** Batch similar periodic checks into `HEARTBEAT.md`. Use cron for precise schedules and standalone pipelines. See [[infrastructure/cron-infrastructure|Cron Infrastructure]] for cron patterns.

---

## The Proof-of-Life Timestamp Pattern

The first action of every Phase 1 heartbeat should be writing a timestamp. This is the foundation of the dead-man's switch monitoring pattern.

```bash
echo $(date +%s%3N) > /tmp/monoclaw-alive.ts
```

An external watchdog process checks this file every 60 seconds. If it's more than 90 minutes old (the heartbeat interval + buffer), the watchdog fires a notification: the heartbeat has stopped.

Why is this the *first* action? Because if the heartbeat fires but fails halfway through, you want the timestamp written. The timestamp means "I started." Not "I finished successfully."

If you want both signals, write two timestamps: `alive-start.ts` at the top and `alive-success.ts` at the bottom. A watchdog that checks both can distinguish "heartbeat started but hung" from "heartbeat never fired."

See [[concepts/dead-mans-switch|Dead-Man's Switch]] for the full watchdog implementation.
...
Simple 60-minute heartbeat. Write a minimal `HEARTBEAT.md` with just the proof-of-life and disk check.
...
```json
{
  "heartbeats": [
    ...
    },
    {
      "name": "dispatch",
      "intervalMinutes": 60, 
      "prompt": "Phase 2 dispatch check. If idle >30min, dispatch next build queue item via Claude Code. Report dispatch status."
    }
  ]
}
```
...
**Doing long work inline.** The heartbeat fires in the main session. If you have it spawn 5 agents and wait for them, you've blocked the session and burned context budget. Heavy work goes to dispatched sessions.

**Pinging for debug output.** "Dispatched task X. Output: [50 lines of logs]." Nobody wants that. Log it, notify only on actionable events.

**Spawning agents in the ops phase.** Phase 1 is monitoring. Spawning agents in Phase 1 means your health check can trigger the condition it's monitoring for.

**No idle check before Phase 2.** If Phase 2 fires while the human is actively talking, it spawns background tasks that compete for resources and potentially interrupt the active session. Always check idle time.

**Missing proof-of-life write.** Without the timestamp, the dead-man's switch can't work. This is the one non-negotiable.
